...
def extract_embedding(frame: np.ndarray, bbox: np.ndarray) -> np.ndarray | None:
    # recorte robusto a fondo y multivista (sólo torso)
    x1, y1, x2, y2 = map(int, bbox)
    h = y2 - y1
    y1 += int(0.15 * h)   # quita cabeza
    y2 = y1 + int(0.60 * h)
    if y2 <= y1:
        return None
...
def find_best_match(
    ...
    pos_th: float = 350.0,   # en la escala de tu homografía, usualmente centímetros
    vis_th: float = 0.90,
) -> int | None:
    ...
    for tid, buf in buffers.items():
        # gating espacial
        if tid in positions and np.linalg.norm(pitch_xy - positions[tid]) > pos_th:
            continue
        d_vis = min(normalized_distance(emb, e) for e in buf)
        if d_vis < vis_th and d_vis < best_score:
            best_id, best_score = tid, d_vis
    return best_id
...
embedding_buffers: Dict[int, deque[np.ndarray]] = defaultdict(lambda: deque(maxlen=20))
...
def process_camera(
    ...
    for bbox in players.xyxy:
        ...
        match_id = find_best_match(
            emb, pitch_xy, embedding_buffers, track_positions
        )
        if match_id is None:
            match_id = next_id
            next_id += 1

        embedding_buffers[match_id].append(emb)
        track_positions[match_id] = pitch_xy
